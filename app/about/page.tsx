...
export const metadata: Metadata = { title: "Always Compliant — About Us", description: "Learn how Always Compliant helps businesses manage GST, ROC, tax, registrations and recurring compliance." };
...
export default function AboutPage() {
  return <PageShell>
    ...
    <section className="ac-section"><div className="container ac-reason-grid"><div><SectionHeading eyebrow="WHY ALWAYS COMPLIANT" title="Support designed around clarity and follow-through." /><img className="ac-reason-visual" src="/assets/img/always-compliant/always-compliant-expertise-orbit.svg" alt="Connected compliance support surrounding a central confirmation mark" /></div><ol className="ac-reason-list">{reasons.map((reason, index) => <li key={reason}><span>{String(index + 1).padStart(2, "0")}</span><h3>{reason}</h3></li>)}</ol></div></section>
